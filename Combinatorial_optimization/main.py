        ...
        for instance in instances:
            ...
            try:
                with multiprocessing.Pool() as pool:
                    tasks = []
                    for tabu_lenght in range(3, 15):
                        for i in range(0, 5):
                            tasks.append((i, adjacency_matrix, 120, tabu_lenght))

                    # Uzywamy imap, aby zapisywac wyniki na biezaco (bezpieczenstwo danych)
                    for result in pool.imap(run_tabu, tasks):
                        i, tabu_lenght, tabu_search_max, _ = result
                        with open(new_file_path, "a") as file:
                            file.write(f"Tabu: {tabu_lenght}-{i}, {tabu_search_max}\n")
                            file.write(f"Greedy: {max_color}\n")
            except Exception as e:
                print(f"Błąd podczas przetwarzania instancji {instance}: {e}")
                with open(new_file_path, "a") as file:
                    file.write(f"ERROR processing {instance}: {e}\n")
